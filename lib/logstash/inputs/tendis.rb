...
module LogStash module Inputs class Redis < LogStash::Inputs::Threadable
  ...
  def load_batch_script(redis)
    #Tendis Lua执行脚本,Tendis不支持load指令,所以此处改为直接赋值
...
EOF
  ...
  def list_batch_listener(redis, output_queue)
    begin
	  #Tendis 2.1.3暂不支持lua脚本,先改为pipelined方式执行
      #results = redis.eval(@redis_script, [@key], [@batch_count-1])
  ...
  def list_single_listener(redis, output_queue)
	#Tendis单条数据拉取,Tendis不支持blpop指令,所以此处改为lpop
    item = redis.lpop(@key)
	unless item then
        sleep BATCH_EMPTY_SLEEP
		return false # from timeout or other conditions
	else
		queue_event(item, output_queue)
	end
...
end end end # Tendis Inputs  LogStash
